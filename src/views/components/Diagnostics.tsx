...
export function Diagnostics({ payload }: { payload?: DiagnosticsPayload }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      patch(
        'mse',
        mseH264 ? 'yes' : mse ? 'partial' : 'no',
        mse ? (mseH264 ? 'H.264 fMP4 supported' : 'MSE present, H.264 unsupported') : 'window.MediaSource undefined',
      );
      patch(
        'mse-worker',
      ...
      patch(
        ...
        hasPresentation
          ? 'PresentationRequest present — but iframe needs allow="presentation" for Google Cast to discover receivers'
          : "Presentation API unavailable — the Cast SDK can't initialize in this sandbox",
      );
  ...
  const testHostFs = async () => {
    ...
    try {
      ...
      setFsHost({
        ...
        detail: ok
          ? 'widget maximized to fill the client window — NOT true OS/desktop fullscreen (video still sits inside the host chrome)'
          : `host did not switch — displayMode stayed "${after}"`,
      });
    } catch (e) {
      setFsHost({
        key: 'fs-host',
  ...
  const testNativeFs = async () => {
    ...
    if (!document.fullscreenEnabled) {
      setFsNative({
        ...
        status: 'no',
        detail:
          'true OS/desktop fullscreen — BLOCKED: document.fullscreenEnabled = false because the widget iframe has no allow="fullscreen" (Permissions Policy), so element.requestFullscreen() is unavailable',
      });
      return;
    }
    if (!el) return;
    try {
      ...
      if (fills) {
        setFsNative({
          ...
          label: 'native Fullscreen API',
          status: 'yes',
          detail: `entered true OS fullscreen — viewport ${w}×${h} matches screen ${sw}×${sh}`,
        });
      } else if (fsEl) {
        setFsNative({
          key: 'fs-native',
          label: 'native Fullscreen API',
          status: 'partial',
          detail: `requestFullscreen() resolved but did NOT fill the screen — viewport only reached ${w}×${h} vs screen ${sw}×${sh}; the host confines "fullscreen" to the widget's iframe area, so there's no real desktop fullscreen`,
        });
      } else {
        setFsNative({
          key: 'fs-native',
          label: 'native Fullscreen API',
          status: 'no',
          detail: 'resolved but no fullscreenElement',
        });
      }
      await sleep(1200);
  ...
  const testPip = async () => {
    ...
    if (!enabled) {
      setPip({
        ...
        status: 'no',
        detail: 'document.pictureInPictureEnabled = false',
      });
      return;
    }
    ...
    if (!v) {
      setPip({
        ...
        label: 'Picture-in-Picture',
        status: 'unknown',
        detail: 'no active <video> — start a DRM playback test below first, then retry',
      });
      return;
    }
    try {
    ...
    <div className="bitflix-root" ref={rootRef}>
      <div className="topbar">
        ...
        <span className="prompt-chip">
          <span className="mic" />
          Video capability probe
        </span>
      </div>
      <div className="headline reveal">
        <h1>Diagnostics</h1>
        <p>
          What does this MCP host's widget sandbox actually allow for video? Results below are probed live in this
          iframe.
        </p>
      </div>
      ...
      <div className="rail reveal">
        <div className="rail-head">
          <h3>Fullscreen &amp; Picture-in-Picture</h3>
          <span className="sub">
            two different things: host displayMode = maximize inside the client window · native API = true OS/desktop
            fullscreen
          </span>
        </div>
        <div className="diag-btn-row">
          <button className="iconbtn accent" onClick={testHostFs}>
            <Icon html={ICON.tv} />
            Fullscreen — host displayMode
          </button>
          <button className="iconbtn" onClick={testNativeFs}>
            <Icon html={ICON.tv} />
            Fullscreen — native API
          </button>
          <button className="iconbtn" onClick={testPip}>
            <Icon html={ICON.cast} />
            Picture-in-Picture
          </button>
        </div>
        <div className="diag-table">
          ...
          {extraRow(pip)}
          {!fsHost && !fsNative && !pip ? (
            <div className="diag-detail" style={{ padding: '6px 2px' }}>
              Current display mode: <b>{displayMode}</b>. Click a test above.
            </div>
          ) : null}
        </div>
      </div>
      ...
      <div className="rail reveal">
        <div className="rail-head">
          <h3>DRM playback</h3>
          <span className="sub">
            which CDM does the sandbox expose? Widevine = Chromium, PlayReady = Edge, FairPlay = Safari
          </span>
        </div>
        <div className="diag-btn-row">
          {drmTitles.map(t => {
            ...
            return (
              <button
                key={t.id}
                className={`iconbtn ${drmPlay?.id === t.id ? 'accent' : ''}`}
                onClick={() => setDrmPlay(t)}
              >
                <Icon html={ICON.play} />
                {label}
                {s ? (
                  <span className={`pill ${s.state === 'ready' ? 'yes' : s.state === 'error' ? 'no' : 'pending'}`}>
                    {s.state === 'ready' ? 'plays' : s.state === 'error' ? 'blocked' : '…'}
                  </span>
                ) : null}
              </button>
            );
